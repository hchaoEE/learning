...
| 跨 die timing graph、TSV/凸点弧、分 die 交付 | 仅 **15** |
| 3D 叠 die / TSV 物理布线 | 03-pnr（综合只写延时标注与 handoff） |

00 章只做 **地图 + 索引**；长文 AIG 细节在 03，00 保留简短 §3 指针即可。

**15 与邻章**：05 只讲 SDC 语法；15 只讲跨 die 约束 **在 timing graph 上的语义**。06 讲通用 transform；15 讲跨 die 路径上 transform **受限/失效**。11 讲通用 budget/abstract；15 讲 **die = 物理分区** 时的 interface model。09 讲 UPF pass；15 仅指针多 die 电压域接口隔离属性。
...
15-<topic>.md     # 选读扩展（接在 14 之后，不打断主链 00→13）
examples/
  <topic>_walkthrough/
```
...
| 15 3D IC | 跨 die 时序、TSV/凸点弧、分 die 交付 | **选读**；06 物理感知 + 11 分块的交叉专题 |

**编号**：10–13 在 **功能综合主链（01–09）之后** 阅读；10、13 对 **所有角色** 建议必读。**14** 在掌握 03–06 后 **选读**，供科研与读论文对照。**15** 在掌握 06、07、11 后 **选读**，面向 Chiplet / 3D 集成项目。
...
| 3D IC / 跨 die 综合专题 | 已写 | 15（timing graph 扩展、06 transform 约束、分 die 交付）；examples/3dic_walkthrough |

## 12. 初学者导读规范（深度细化）

00–15 正文统一结构（与机制正文并存，不删表/案例）：
