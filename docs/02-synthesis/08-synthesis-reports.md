...
> **本章回答**：报告里的 WNS/面积/功耗对应 DB 里哪些内部量。
...
## 7. QoR 对比（版本间）注意
> **一句话**：QoR 对比（版本间）注意——本章核心机制点。

### 输入/输出案例 7.1 — 假变：SDC 未对齐

**输入**：v1 WNS=+0.05；v2 仅改 `create_clock -period 0.9`（其余相同网表）。

| 对比项 | v1 | v2 | 可否比 |
|--------|----|----|--------|
| period | 1.0 | 0.9 | — |
| WNS | +0.05 | −0.12 | **不可**（约束变了） |
| instance 数 | 12k | 12k | 可比 |

**输出**：报告对比前须锁定 **SDC / lib / MCMM** 表（本章 §7 表）。
