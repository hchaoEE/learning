...
### 输入/输出案例 2.1 — miter 最小例

**输入**：R、I 均为 `y = a & b`；miter 在 `y` 上 XOR。

**输出**：strash 后 XOR 两输入合一 → **proven**（无 SAT）；若 I 为 `y = a & !b` → SAT 赋值 `a=1,b=0` → **falsified**。

---

## 3. Compare point 匹配（内部）
> **一句话**：Compare point 匹配（内部）——本章核心机制点。
> **类比**：像两版图纸叠在一起，用 XOR 灯看哪里不一致。
...
**初学者易错**：LEC 失败先怀疑「综合改坏了逻辑」——多数是先查 **unmatched / 缺变换日志**（ungroup、常量传播、FSM 重编码），匹配修好再谈 miter 不等价。

### 输入/输出案例 3.2 — 常量传播 unmatched

**输入**：R 有 `reg_x`；I 经常量传播 **删除** `reg_x`（tie 0）。

**输出**：compare point **unmatched**（非 falsified）→ 须变换日志标 `merged/tied` 或常量感知匹配（[02 §10](./02-inference.md#10-寄存器级优化-pass推断后的时序元件清理)）。
...
### 输入/输出案例 5.1 — SAT 管线四步（概念网表）

**输入**：R、I 各 `y = a & b | c`；miter 在 `y` 上 XOR。

| 步骤 | 内部状态 | 结果 |
|------|----------|------|
| ① strash | R、I 同结构子锥合并 | 若两侧 **完全相同** → XOR 输入同一节点 → **proven（无 SAT）** |
| ② Tseitin | AIG 8 节点 → CNF ≈ 8 变量 + 24 子句 | 规模线性 |
| ③ SAT | diff=1 目标子句 | **UNSAT** → proven |
| ④ 若 I 中 `b` 极性接反 | SAT 赋值 `a=1,c=0` | **falsified** → §7.1 |

### 输入/输出案例 5.2 — 证明缓存复用

**输入**：compare point `reg_a/Q` 与 `reg_b/Q` 的 fanin 锥 **共享同一乘法器子锥**（AIG 哈希相同）。

| 次序 | 动作 | CNF 规模 |
|------|------|----------|
| 先证 `reg_a/Q` | 乘法器子锥 UNSAT → **缓存签名 H** | 50k 子句 |
| 再证 `reg_b/Q` | 命中 H → **黑盒跳过** | 12k 子句（仅差异锥） |

### 输入/输出案例 5.3 — 四态与 abort

| compare point | 状态 | 交付 |
|---------------|------|------|
| `u_alu/out` | **aborted**（锥>2M 门） | 须 waiver 或缩 scope（§8） |
| `u_ctrl/q` | **proven** | pass |
| `u_mux/y` | **inconclusive**（induction 不闭合） | **不是 pass** |

### 输入/输出案例 5.4 — k-induction 不闭合

**输入**：R 用 one-hot FSM；I 经 re-encoding 为 binary（[02 §7.3](./02-inference.md#73-状态重编码re-encoding)），无 state mapping。

**输出**：BMC k=10 无反例 → **仍 inconclusive**；须补变换日志 state map 或 STG 级比对。

详见 [lec_walkthrough/](./examples/lec_walkthrough/README.md)。

---

## 6. 综合引导信息（Transformation Log）
> **一句话**：综合引导信息（Transformation Log）——本章核心机制点。
...
### 输入/输出案例 8.1 — 子块 proven 后顶层 miter 缩小

**输入**（[examples/hier_walkthrough/](./examples/hier_walkthrough/README.md)）：`cpu_core` 已 proven；顶层仅 `glue` + 接口。

| 层级 | miter 规模（概念） | compare point |
|------|-------------------|---------------|
| 块 `cpu_core` | 50k 门 | 块内全部 FF/PO |
| 顶层（块黑盒后） | **800 门** | 仅 `cpu_core/inst_*` 边界 + top glue |

**输出**：顶层 LEC 时间从「不可行」→ 分钟级；块内变更须 **重证块** 再重跑顶层。

---

## 9. Retiming 与 LEC
> **一句话**：Retiming 与 LEC——本章核心机制点。
...
### 输入/输出案例 9.1 — pipeline 等价

**输入**：R 1 级 FF；I 经 [06 retime](./06-timing-driven-optimization.md#8-retiming寄存器搬移--流水线重平衡) 变 2 级 FF，组合功能不变。

**输出**：组合 LEC **unmatched**（FF 数不同）；pipeline 模式 + `latency=1` 约束 → **proven**（见 [06 案例 8.2](./06-timing-driven-optimization.md#输入输出案例-82--retiming-vs-rtl-流水)）。
