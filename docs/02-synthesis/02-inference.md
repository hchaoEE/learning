...
Re-encoding 后 R 与 I 的 state 寄存器 **逐位含义不同** → [09 章](./09-logical-equivalence-checking.md) 名字/拓扑匹配失败。需要：

- 综合导出 **state mapping**（变换日志的一部分，09 §6）
...
**与 09 LEC 的关系**：这三类 pass 是 **unmatched / merged compare point 的主要来源** — R 侧寄存器在 I 侧消失或被合并，LEC 须依赖变换日志（[09 §6](./09-logical-equivalence-checking.md#6-综合引导信息transformation-log)）或常量/合并感知匹配，否则报 unmatched 而非真正不等价。
