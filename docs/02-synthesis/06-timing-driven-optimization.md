...
> **本章回答**：映射后引擎如何用 slack 驱动 upsize/buffer 等 transform。
...
### 输入/输出案例 1.1 — 映射后首轮修 setup

**输入**（04 映射刚结束）：时序图上 worst setup slack = **−0.10 ns**（见 §2.1 案例的算式）。

**输出**（06 若干轮 transform 后）：同一 check 的 slack = **+0.02 ns**；网表 instance 数略增（buffer / 更大驱动单元）。

---

## 2. 细粒度优化引擎（内部）
> **一句话**：STA 算 slack → 违例扫描 → 选 transform → 增量 STA 闭环，直到预算耗尽。
> **类比**：像交通调度：先清障（DRC），再疏通最堵路口（WNS），最后扫尾小堵点（TNS）。

04 映射结束时，Design DB 中已有 **mapped netlist**。06 的核心是一个 **STA ↔ transform 闭环**（STA 引擎本身 — graph 构建、AT/RT 传播、check 求值、QoR 聚合 — 见 [07 章](./07-internal-sta-and-qor.md)；本章聚焦 **transform 侧**）：
...
### 2.1 时序图：transform 能改哪些边

图结构与传播算法见 [07 §2–§3](./07-internal-sta-and-qor.md#2-timing-graph-数据结构)。06 关心的是 **每类边的可改性**：
...
| **Check 边**（setup/hold 库值） | FF 的 `.lib` 查表 | **间接** — FF 换型连带改 check 值 |

**Slack 速记**（完整定义见 [07 §3.3](./07-internal-sta-and-qor.md#33-slack)）：
...
|------|-----------|-----------|--------|
| Cell | .lib NLDM 表 | 随 sizing **换表** | 同左 + OCV/AOCV（综合侧仅 global derate，[07 §7](./07-internal-sta-and-qor.md#7-derate--ocv概念层)） |
...
### 输入/输出案例 2.2 — 三层 delay 不同步更新

**输入**：一轮 upsize 后仅 **cell arc** 换表；WLM **未**变。

| 层次 | 本轮是否重算 | 原因 |
|------|--------------|------|
| Cell | ✓ | ref 变化 → NLDM 重查 |
| Net | ✗ | fanout 拓扑未变（同 WLM） |
| Clock | ✗ | 仍 ideal |

**输出**：若后续插 buffer → net 拓扑变 → **WLM 段 delay 全变**。
...
| **Multibit banking / debank** | 单比特 FF ↔ 多位 FF 单元（见 §2.7） | clock pin 负载、个别 bit 路径 | 功耗/面积模式；bank 内 bit 违例时拆出 |
...
### 输入/输出案例 2.3 — transform 指纹

**输入**：hold 违例 on `reg_b/D`。

| Transform | instance Δ | buffer 占比 Δ | 典型场景 |
|-----------|------------|---------------|----------|
| upsize | 0 | 0 | setup |
| buffer insert | +2~4 | ↑ | DRC / fanout |
| DELAYX1 | +1 | 0 | hold only |
| VT swap | 0 | 0 | leakage 权衡 |
...
| 阶段 | 驱动指标 | 行为 |
|------|----------|------|
| 严格顺序：先 DRC | transition/cap 违例数 | slew 超限时 delay 表外推不可信（§5.1）→ **任何 timing 数字都不算数**，必须先修 |
| 中段：WNS 驱动 | 各桶 worst path（[07 §5](./07-internal-sta-and-qor.md#5-路径分组与-endpoint-桶) 桶权重排序） | 集中攻最深的锥；适合「单点瓶颈」形态 |
| 后段：TNS 驱动 | 全部负 endpoint 按 slack 排序批量处理 | WNS 已接近 0 但违例点多时切换；批量小修比逐点更高效 |
| 回滚与黑名单 | 失败 transform 记录 | 同一位置同一候选不重试；连续失败 N 次的 endpoint 降级（留给 remap / 人工） |

**何时回 04 remap（而非继续 sizing）**：sizing/buffer 是 **同 cover 内** 的微调；当 ① 关键锥上所有 cell 已 upsize 触顶、② buffer 只在搬运 slack 不再产生净收益、③ WNS 缺口 > 单级 cell delay 时，引擎对该锥触发 **局部 remap**——回到 [04](./04-technology-mapping.md) 对违例锥重新 cut/cover（可能换 AOI 结构、改级数），但 **不回 03 的 AIG rewrite**（compile 单轮内布尔结构已冻结；多轮 compile_ultra 迭代才会重进粗优化，见 [00 §8](./00-synthesis-overview.md#8-compile-内部-pass-时间线全景)）。

**初学者易错**：把「WNS 已接近 0」当成可以停——若 **TNS 仍大** 或 **hold/fast corner** 未过，签核仍会挂；调度队列会在中段 WNS 与后段 TNS 之间切换，不是修一条路径就结束。

### 输入/输出案例 2.4 — WNS 桶 vs TNS 桶调度
...
| `clk/reg2reg` | **−0.12** | 1 |
| `in2reg` | −0.28 | 1 |
| 全芯片 TNS | −3.50 | 80（大量 −0.04 小违例） |

**输出（调度决策）**：

| 阶段 | 动作 | 原因 |
|------|------|------|
| 第 1–5 轮 | 修 `reg2reg` 桶 −0.12 路径 | 桶权重高；全局 WNS 被 in2reg 误导 |
| WNS 升至 −0.02 后 | 切换 **TNS 模式**：批量处理 80 个 −0.04 | 单点瓶颈已消，批量 downsize/upsize 更高效 |
| 全程 | `in2reg` −0.28 **排队靠后** | input_delay 常为系统级假设，先核对 SDC |

详见 [tdo_walkthrough 案例 D](./examples/tdo_walkthrough/README.md#案例-d--调度策略0624)。

### 2.5 增量 STA（transform 侧视角）

每做一次 transform 都跑 **全芯片 STA** 代价过高；引擎只重算 **失效区域**（脏标记传播、slew 扩散、ε 截断 — 机制见 [07 §8](./07-internal-sta-and-qor.md#8-增量-sta-机制)）。06 侧关心两点：

- **接受判定读增量结果**：transform 后仅受影响 endpoint 的 slack 更新，QoR（WNS/TNS）增量维护 — 全 corner 不恶化才接受（§4.2）
- **代价模型**：失效区域大小 ≈ transform 影响半径；插 buffer 比换 ref 半径大（新增 node/edge + 负载拆分）

这使 06 能在 compile 迭代中跑 **数百轮** 局部 transform。

### 输入/输出案例 2.5 — 增量 STA 脏区

**输入**：`u2` upsize（案例 2.6 路径上 1 个 cell）。
...
```systemverilog
// launch FF（reg_a）→ 5 级组合 AND 链 → capture FF（reg_q）
always_ff @(posedge clk) a_r <= a;
assign w5 = ((((a_r & b) & c) & d) & e) & f;
always_ff @(posedge clk) q <= w5;
```

**04 映射后网表（片段）**：

```text
reg_a/Q ──► ND2D1 ──► ND2D1 ──► ND2D1 ──► ND2D1 ──► ND2D1 ──► reg_q/D
            u1        u2        u3        u4        u5
```
...
| 弧 | delay | 备注 |
|----|-------|------|
| clk→reg_a/Q | 0.12 | launch |
| 每级 ND2D1 | 0.18 | 5 级 ≈ 0.90 |
| reg_q/D setup | 0.08 | capture 要求 |
| **clock period** | 1.00 | SDC |

**Slack**（按 §2.1 定义两步算）：

```text
arrival  @reg_q/D = 0.12 (clk→Q) + 0.90 (组合) = 1.02
required @reg_q/D = 1.00 (period) − 0.08 (setup) = 0.92
slack = 0.92 − 1.02 = −0.10   （违例，与 §1 案例一致）
```

### 输入/输出案例 2.7 — 一轮 upsize 后

**Transform**：路径上 2 个 `ND2D1`（u2、u3）→ `ND2D4`（cell arc 各 −0.04 ns）。
...
|------|----|----|
| 组合段 delay | 0.90 | 0.82 |
| arrival @reg_q/D | 1.02 | 0.94 |
| WNS | −0.10 | −0.02 |

引擎 **未停止**：仍负，继续选 buffer 或 further upsize（如 u5 再 upsize → WNS +0.02），直至 WNS ≥ 0 或 effort 用尽。
...
### 2.7 Multibit FF banking / debanking

**Banking**：把多个 **同 clock、同复位/使能、placement 邻近**（物理感知时）的单比特 FF 合并为一个 **多位单元**（`DFF2X*`/`DFF4X*`）：

```text
4× DFFX1（各自 CK pin）──bank──► 1× DFF4X1（共享 CK 内部缓冲）
```

| 维度 | 收益 / 代价 |
|------|--------------|
| Clock pin 电容 | 4 个 CK 负载 → 1 个 → **clock 树功耗 ↓**（与 [09](./09-low-power-synthesis.md) §6 活动度模型呼应） |
| 面积 | 多位单元 < N 个单比特之和（共享 well/缓冲） |
| 时序 | 各 bit 的 D/Q **被绑到同一位置** → 个别 bit 路径可能恶化 |

**Debanking** 是反向 transform：bank 中某 bit 成为关键路径端点时，**拆出该 bit** 单独放置/sizing。两者都是 §2.3 意义上的局部 rewrite（换 ref + 重连 pin），跟随增量 STA。

| 触发 | 方向 |
|------|------|
| 功耗/面积模式、非关键区域 | banking |
| bank 内某 bit setup 违例且 sizing 无效 | debanking |
| 物理感知（§6）密度/邻近度变化 | 双向重评估 |

### 输入/输出案例 2.10 — banking 前后

**输入**：32 位数据寄存器，全部非关键，单比特 `DFFX1` ×32。

| 指标 | 前 | 后（8× `DFF4X1`） |
|------|----|--------------------|
| CK pin 数 | 32 | 8 |
| clock net cap（相对） | 1.0 | ~0.4 |
| 面积（相对） | 1.0 | ~0.85 |
| WNS | +0.20 | +0.16（仍正 → 接受） |
...
## 3. Setup 修复（内部）
> **一句话**：Setup 修复（内部）——本章核心机制点。
> **类比**：像交通调度：先清障（DRC），再疏通最堵路口（WNS）。
...
04 用 **arrival/required 双端**（[04 §4.4](./04-technology-mapping.md#44-延时驱动-vs-面积驱动)）选 cover；06 发现 **实际 WNS 仍负** 时，先在 **已选 cover 之上** 做 **局部 sizing**；sizing 触顶后可对违例锥 **局部 remap**（§2.4），但不回到 AIG rewrite（那是 03 的事）。

### 输入/输出案例 3.2 — 现象→transform 映射

**输入**：违例路径 `u_u2/Z` slack 贡献 −0.12 ns，fanout=1。

**输出**：队列 **#1 upsize u_u2**（非 buffer）；若 fanout=18 则 **#1 buffer tree**。

---

## 4. Hold 修复（内部）
> **一句话**：Hold 修复（内部）——本章核心机制点。
> **类比**：像交通调度：先清障（DRC），再疏通最堵路口（WNS）。
...
```text
launch ──► 2 级 ND2 ──► capture FF/D     （同沿捕获、零 skew 假设）
min arrival（fast corner，含 clk→Q）= 0.08 ns
capture FF hold_time = 0.15 ns
→ slack_hold = arrival − hold_time = 0.08 − 0.15 = −0.07 ns
```

**Transform**：capture 前插入 `DELAYX1`（min delay +0.10 ns），setup 路径仅 +0.02 ns → 两 corner 均闭合。

### 输入/输出案例 4.2 — 双 corner 接受判定

**输入**：transform T：slow setup +0.10，fast hold −0.02。

**输出**：`reject T`（hold 恶化）；接受条件见上式 — **任一 corner 恶化即回滚**。
...
### 输入/输出案例 4.3 — upsize 触发双 corner 回滚

**输入**（复用 [hold_short_path.sv](./examples/tdo_walkthrough/hold_short_path.sv)）：`reg_a/Q ── ND2D4 ── reg_b/D`，period = 1.0 ns。

| 轮次 | Transform | slow_max setup | fast_min hold | 接受？ |
|------|-----------|----------------|---------------|--------|
| 0 | — | −0.05 | +0.02 | — |
| 1 | u1 ND2D4→ND2D8（修 setup） | +0.08 | **−0.06** | **回滚** |
| 2 | reg_b/D 前插 DELAYX1 | +0.06 | +0.04 | ✓ |

**输出**：引擎日志 `reject T1: hold degraded on fast_min` → 黑名单 `(u1, upsize ND2D8)` → 改试 hold 专用 transform。与 [05 案例 6.1](./05-constraints-sdc.md#输入输出案例-61) 同一 net `n48` 叙事一致。

---

## 5. 转换与电容 DRC（内部）
> **一句话**：转换与电容 DRC（内部）——本章核心机制点。
> **类比**：像交通调度：先清障（DRC），再疏通最堵路口（WNS）。
...
### 输入/输出案例 5.1 — DRC 优先于 timing

**输入**：同时存在 `transition_violation` on `n_wide` 与 `setup_violation` on `reg_q/D`。

**输出**：队列 **先** `n_wide` buffer tree，**后** setup upsize（与 05 §7.1 一致）。
...
### 输入/输出案例 5.2 — fanout=32 buffer 树
...
## 7. 与 05 约束、08 报告内部量
> **一句话**：与 05 约束、08 报告内部量——本章核心机制点。

| 模块 | 06 消费什么 |
|------|-------------|
| [05 SDC](./05-constraints-sdc.md) | clock 边、IO delay、false/multicycle → **required time** |
| [08 报告](./08-synthesis-reports.md) | WNS/TNS、buffer 占比 — 反映 **哪类 transform** 曾大量发生 |
...
> **一句话**：Retiming（寄存器搬移 / 流水线重平衡）——本章核心机制点。

**Retiming** 在 **不改变每个逻辑锥布尔功能** 的前提下，**移动寄存器位置**（或增删寄存器），以 **平衡组合逻辑级数**、改善 setup。**属于时序驱动的顺序优化**，在 **mapped 网表** 上执行，与 [03](./03-optimization.md) 的 AIG 组合优化 **正交**。注意：逐锥组合功能不变，但 **I/O 周期延迟（latency）可能改变**（见 §8.1 表）。

> 配套案例：[examples/retiming_walkthrough/](./examples/retiming_walkthrough/)（`long_comb.sv` 的 retime 前后 timing graph 对照）。
...
### 输入/输出案例 8.2 — retiming vs RTL 流水

**输入**：[long_comb.sv](./examples/retiming_walkthrough/long_comb.sv)，WNS=−0.20。

**输出**：06 retime 插入 1 级 FF → 组合 depth 减半、latency +1；与 RTL 手写流水等价但 **无 RTL 改动** — LEC 走 pipeline 模式（[10 §9](./10-logical-equivalence-checking.md)）。
...
```text
原  ：a ──► h ──► g ──► f ──► FF ──► q              （1 周期，组合过深）
retime：a ──► h ──FF── g ──FF── f ──FF── q          （3 周期 latency，每段组合变浅）
```
...
| 组合 depth | 整条路径 | 分段 |
| WNS | -0.2ns | +0.1ns（示意） |
| 周期延迟（I/O） | 1 cycle | **3 cycles（示意）** — 与 §8.4 案例的「1 级 FF → 3 级 FF」对应 |

**若系统要求固定 latency**：RTL 固定流水级，或在 DB 上对 instance 设 **dont_retime** 属性（综合器跳过该 FF）。

### 输入/输出案例 8.3 — retiming 与 sizing 分工
...
### 输入/输出案例 8.4 — dont_retime 隔离

**输入**：FIFO 指针逻辑带 dont_retime。

**输出**：retime 仅发生在 datapath；指针 **FF 拓扑不变**，时序图该子图 **隔离**。

### 8.4 与 LEC（[10 章](./10-logical-equivalence-checking.md)）
...
见 [12 DFT](./12-dft-and-scan.md)、[11 层次](./11-hierarchical-block-synthesis.md)。
...
| 收益停滞 | 连续 N 轮 ΔWNS < ε（QoR 聚合见 [07 §6](./07-internal-sta-and-qor.md#6-qor-聚合)）→ 升级 effort / 局部 remap |
...
- [08 综合报告](./08-synthesis-reports.md)
