...
**何时回 04 remap（而非继续 sizing）**：sizing/buffer 是 **同 cover 内** 的微调；当 ① 关键锥上所有 cell 已 upsize 触顶、② buffer 只在搬运 slack 不再产生净收益、③ WNS 缺口 > 单级 cell delay 时，引擎对该锥触发 **局部 remap**——回到 [04](./04-technology-mapping.md) 对违例锥重新 cut/cover（可能换 AOI 结构、改级数），但 **不回 03 的 AIG rewrite**（compile 单轮内布尔结构已冻结；多轮 compile_ultra 迭代才会重进粗优化，见 [00 §7](./00-synthesis-overview.md#7-compile-内部-pass-时间线全景)）。
