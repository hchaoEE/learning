...
1. 先攻 reg2reg（桶权重），忽略全局 WNS 被 in2reg 拉低  
2. reg2reg 闭合后切 **TNS 批量**修 80 个小违例  
3. in2reg 最后核对 `set_input_delay` 是否过悲观  
...
```text
06 §1 对比 03 → §2 引擎 → 案例 A → §2.4 D → §3 setup
                              → 案例 B → §4.3/4.1 hold → §5 DRC
```
