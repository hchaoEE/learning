...
下文各节 **输入/输出案例** 均以此为主线；阶段 IR 对照见 [elab_walkthrough/README.md](./examples/elab_walkthrough/README.md)。
...
> **一句话**：内部核心数据结构（Design Database）——本章核心机制点。

综合器在内存中维护一张 **异构图（heterogeneous graph）**，仿真器也有类似层次，但综合侧 **二值逻辑、无延时、无 initial 执行**。

### 2.0 异构图（heterogeneous graph）是什么

**图（graph）** 在这里指：用 **节点** 存对象、用 **边** 存关系，在内存里表示整张设计。**异构** 的意思是：节点和边 **不止一种类型**，每种类型携带 **不同的语义与字段**；遍历、合法性检查、序列化都要 **按类型分派**，不能像「所有节点都是同一种门」那样用一套规则走天下。

```text
  同构图（homogeneous）          异构图（heterogeneous）
  ─────────────────────          ──────────────────────
  节点：全是 2-input AND         节点：Design / Cell / Pin / Net / Port …
  边：全是「信号连线」            边：实例化(ref)、连接(connect)、层次(parent) …
  典型 IR：AIG（03 章）           典型 IR：Elaboration 后的 Design DB（本章）
```

| 对比项 | 异构图（Design DB） | 同构图（如 AIG） |
|--------|---------------------|------------------|
| **节点种类** | Module、Cell、Pin、Net、Port… 多种 | 几乎只有 AND（反相画在边上） |
| **边种类** | `ref`（Cell→模板）、`connect`（Pin→Net）、父子层次… | 主要是布尔依赖边 + 互补标记 |
| **保留的信息** | 层次路径、总线范围、端口方向、实例参数 | 纯组合布尔函数，层次已展平 |
| **主要用途** | Elaboration、Link、多驱动/悬空检查、报告点名 | 布尔化简、工艺映射前的优化 |
| **出现阶段** | 前端 elaborate 起贯穿到 mapped 网表 | 03 粗优化起，映射前后仍以同构布尔图为主 |

**Design DB 里「异构」体现在哪**：

1. **多类节点**：`Cell` 是实例，`Module` 是模板，`Pin` 是连接点，`Net` 是导线——它们不能混为一种「通用节点」；工具要对 `Cell` 查 `ref`，对 `Pin` 查 `Net`，对 `Net` 查驱动/负载列表。
2. **多类边**：`u_child` **引用** `child` 模块是 **ref 边**；`pin din → data_in` 是 **connect 边**；`Design → Cell` 是 **层次包含边**。三类边的遍历方向与合法性规则不同。
3. **节点上挂不同属性**：`Net` 可有 bus range、`Cell` 可有 `W=8` 参数落地值、`Port` 有 `input/output` 方向——同一张图里 **schema 随类型而变**。

**为何 Elaboration 阶段必须用异构图**：RTL 首先是 **结构 + 层次 + 类型**（谁例化谁、哪根线连哪个 pin、总线位宽多少），而不是单纯的布尔公式。用异构图可以在 **不丢层次名** 的前提下做 `check_design`（多驱动、悬空、位宽不匹配）；若一上来就压成 AIG 同构图，层次路径与接口信息会丢失，后续 DFT、层次 LEC、按 block 签核都会很痛苦。

**与同构图 IR 的衔接**（见 [03 章](./03-optimization.md)）：Lowering 到 GTECH 后，**组合逻辑锥** 可再被抽成 AIG 做重写；但 **寄存器（SEQGEN）、算术壳、层次边界** 往往仍以异构节点形式挂在 Design DB 上。可以记一句口诀：**前端管「谁连谁」，中端管「布尔怎么算」**——前者天然异构，后者为优化而尽量同构。

```text
  RTL 文本
      │ elaborate（异构图 Design DB：Cell/Pin/Net/层次）
      ▼
  GTECH 网表（仍以异构节点为主，门类型增多）
      │ 组合锥提取
      ▼
  AIG（同构图：AND + 边反相）→ 优化 → 映射 → 回到门级异构网表
```
...
### 输入/输出案例 8.4 — `unique case` lowering
...
### 输入/输出案例 8.5 — `*` lowering 为 GTECH_MULT
...
| 对象 | 类型 | 说明 |
|------|------|------|
| `p` 驱动锥根 | `GTECH_MULT` | 位宽 16×16→32；**不进 AIG 拆解**（§8.5 边界） |
...
### 输入/输出案例 11.1 — 仿真 vs 综合 elaboration
...
→ 阶段 IR 表：[elab_walkthrough](./examples/elab_walkthrough/README.md)；全链：[mini_chain](./examples/mini_chain/README.md)。
