...
[06 章 retiming](./06-timing-driven-optimization.md#8-retiming寄存器搬移--流水线重平衡) 会 **增删/移动 FF**，比对点与 RTL **数量可能不同**。

| 情况 | 做法 |
|------|------|
| 综合开启 retime + SVF | Formality `read_svf` 识别搬移 |
| RTL 固定 latency | 综合 `set_dont_retime` 或 RTL 已流水 |
| 失败：FF 数不等 | 配置 **retiming-aware** 验证，非直接放弃 |

### 输入/输出案例

**输入**：RTL 10 万 FF，网表 10.2 万 FF（retime 插入 2000）

**输出**：带 SVF 的 **sequential equivalence PASS**；无 SVF 时 thousands **unmatched**。
